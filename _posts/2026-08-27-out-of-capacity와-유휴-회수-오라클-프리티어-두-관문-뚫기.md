...
CPU를 억지로 태우는 스크립트를 돌리는 사람도 있는데, 전력 낭비인 데다 메모리 조건 하나로 해결되므로 권하지 않는다. 실제로 시간당 `stress-ng` 버스트를 4개월간 돌려본 사후평가가 [6편](/dev/2026/08/27/유휴-회수-방어-4개월간-헛돌았다-stress-ng-크론잡-사후평가/)에 있는데, 결론은 **그 방식이 95 퍼센타일 기준을 아예 못 넘긴다**는 것이다.

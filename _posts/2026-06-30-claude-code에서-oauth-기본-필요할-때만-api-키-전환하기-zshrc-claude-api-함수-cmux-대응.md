---
layout: single
title: "Claude Code에서 OAuth 기본 + 필요할 때만 API 키 전환하기 (zsh/bash claude-api 함수, cmux 대응)"
date: 2026-06-30 23:24:29 +0900
categories: [dev]
tags: [Claude, Claude Code, zsh, bash, Linux, cmux, iTerm2, ANTHROPIC_API_KEY, OAuth, macOS, dotfiles]
excerpt: "평소엔 OAuth 구독 인증으로 claude를 쓰고, 특정 작업에서만 claude-api 명령으로 API 키를 임시 적용하는 셸 설정. macOS(zsh)·리눅스(bash)·iTerm2·cmux 모두에서 동작하도록 셸 자동 감지와 cmux 래퍼/shim 우회까지 절차적으로 정리했다."
---
평소에는 **OAuth 로그인(구독 인증)** 으로 `claude`를 쓰고, 특정 작업에서만 `claude-api` 명령으로 **`ANTHROPIC_API_KEY`(API 키 인증)** 를 임시 적용하는 방법을 절차적으로 정리한다.
macOS(zsh) · 리눅스(bash) · iTerm2 · cmux(터미널 멀티플렉서) 모두에서 동일하게 동작한다. (함수가 실행 중인 셸을 자동 감지)
...
```zsh
# 현재 셸 확인 — zsh(macOS 기본) / bash(리눅스 등) 모두 지원
echo $SHELL            # → /bin/zsh, /usr/bin/zsh, /bin/bash, /usr/bin/bash 등

# Claude Code 설치 및 경로 확인
claude --version
which claude           # 예: /opt/homebrew/bin/claude (리눅스는 /usr/local/bin/claude 등)
```
...
> ⚠️ 키 끝에 줄바꿈/공백이 들어가지 않도록 주의. `echo`로 넣으면 끝에 개행이 하나 붙지만, 함수가 `$(<file)`로 읽을 때 zsh·bash 모두 명령 치환에서 마지막 개행을 제거하므로 문제없다.

---

## 4. `.zshrc`(또는 `.bashrc`)에 `claude-api` 함수 추가

셸 설정 파일 하단에 아래 블록을 추가한다 (편집기로 열거나 그대로 붙여넣기). zsh는 `~/.zshrc`, bash(리눅스 등)는 `~/.bashrc`에 넣으면 된다. 아래 함수는 실행 중인 셸을 자동 감지해 zsh·bash 모두에서 동일하게 동작한다.

```bash
# ===== Claude Code: API 키로 임시 실행 (zsh / bash 공용) =====
...
claude-api() {
  ...
  # 2) 키 내용 읽기 + 빈 값 확인  ($(<file)은 zsh·bash 모두 지원)
  key="$(<"$key_file")"
  if [[ -z "$key" ]]; then
  ...
  #    실제 바이너리를 직접 실행한다.
  #    (cmux가 없는 iTerm2/일반 zsh·bash에서는 매칭이 없어 평소대로 동작)
  #    PATH의 실제 바이너리 나열 명령이 셸마다 다르다: zsh=whence -ap / bash=type -aP
  local real_claude
  if [[ -n "${ZSH_VERSION:-}" ]]; then
    real_claude="$(whence -ap claude | grep -v 'cmux-cli-shims' | head -1)"
  else
    real_claude="$(type -aP claude | grep -v 'cmux-cli-shims' | head -1)"
  fi
  ...
  (
    ...
    # zsh 전용 배열 문법 대신, zsh·bash 공용으로 PATH 문자열을 필터링한다.
...
> zsh 전용이던 부분을 두 셸 공용으로 바꾼 곳은 세 군데다:
>
> - **바이너리 나열**: `whence -ap`(zsh 내장) → `$ZSH_VERSION` 유무로 분기해 bash에서는 `type -aP` 사용
> - **PATH에서 shim 제거**: zsh 배열 문법 `path=("${(@)path:#...}")` → `tr`/`grep`/`paste`로 PATH 문자열 직접 필터링(두 셸 공용)
> - 나머지(`[[ ]]`, `local`, `$(<file)`, `( ... )` 서브셸)는 bash에서도 그대로 동작한다.
...
| 3 | 셸에 맞는 명령(zsh `whence -ap` / bash `type -aP`)으로 PATH의 실제 바이너리만 나열 → cmux shim 제외 → 진짜 `claude` 선택 |
...
| 구문 | 역할 |
|------|------|
| `[[ -n "${ZSH_VERSION:-}" ]]` | 실행 중인 셸이 zsh인지 판별 (bash면 `else` 분기) |
| `whence -ap claude` (zsh) | PATH의 **실제 바이너리만** 나열 (함수/alias 무시) |
| `type -aP claude` (bash) | zsh `whence -ap`의 bash 대응 — PATH의 실제 바이너리만 나열 |
...
| `PATH="$(... tr ':' '\n' \| grep -v ... \| paste -sd ':' -)"` | 서브셸 PATH에서 shim 디렉터리 제거 (중첩 호출 대비, 없으면 무해). zsh 배열 문법 대신 두 셸 공용 문자열 필터 |
| `"$real_claude" "$@"` | 절대경로로 진짜 바이너리 직접 실행, 인자 그대로 전달 |

> 경로를 하드코딩하지 않고 PATH에서 찾으므로 Intel맥(`/usr/local/bin`)·리눅스 배포판·버전 변경에도 자동 대응한다.
...
```bash
# bash (리눅스 등) — 함수를 ~/.bashrc에 넣은 경우
bash -n ~/.bashrc      # 문법 오류 없는지 먼저 확인
source ~/.bashrc       # 새 함수 정의를 현재 셸에 반영 (또는 새 터미널 열기)
```
...
## 7. 환경별 동작 (iTerm2 / 일반 zsh / bash·Linux / cmux)

| 환경 | `claude-api` | 평소 `claude` |
|------|--------------|----------------|
| iTerm2 / 일반 zsh | shim 없음 → 진짜 바이너리 직행, 키 상속 ✅ | OAuth 구독 |
| bash / Linux | shim 없음 → 진짜 바이너리 직행, 키 상속 ✅ | OAuth 구독 |
| cmux | shim 우회 → 진짜 바이너리 직접 실행, 키 상속 ✅ | OAuth 구독 (cmux 통합 유지) |

셸 종류(zsh/bash)와 환경을 분기하지 않고 한 함수로 모두 커버한다. 셸은 `$ZSH_VERSION`으로 자동 감지하고, cmux가 있으면 우회하며 없으면 우회 로직이 매칭되지 않아 무해하다.
...
| 증상 | 원인 / 해결 |
|------|-------------|
| `claude-api: command not found` | 설정 파일이 안 읽힘. `source ~/.zshrc`(bash는 `source ~/.bashrc`) 또는 새 터미널. interactive 셸인지 `echo $-`로 `i` 확인 |
| `❌ 키 파일이 없습니다` | 3장 키 파일 생성 안 됨 |
| `❌ 키 파일이 비어 있습니다` | 키 파일에 실제 키가 안 들어감 (`cat ~/.secrets/anthropic_key`로 확인) |
| `claude-api`인데 여전히 구독 인증 | cmux 환경에서 우회가 안 됨 → 실제 바이너리 목록(zsh `whence -ap claude` / bash `type -aP claude`)에 shim이 섞여 있는지, 함수에 우회 로직(4장 3단계)이 포함됐는지 확인 |
...
또는 매번 직접 입력(가장 안전, 가장 번거로움) — `read`의 프롬프트 문법도 셸마다 다르다:

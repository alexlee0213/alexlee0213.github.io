...
Claude 데스크톱 앱에서 Slack으로 메시지를 전송할 때, 발신자가 기존 'Alex'에서 'Charles'로 변경된 것이 확인되었습니다. 이 이슈를 계기로 Slack MCP 커넥터의 동작 원리를 조사하고, 발신자 계정이 결정되는 메커니즘을 정리하였습니다.
...
6. **토큰 저장:** Anthropic 서버가 토큰을 암호화하여 저장하고, Claude 사용자 계정(ai_d\*\*@catch\*\*\*\*\*.com)과 매핑합니다. 이후 모든 Slack API 호출에 이 토큰이 사용됩니다.
...
예시: 사용자가 "danny에게 슬랙 메시지 보내줘: 내일 미팅 가능?"이라고 입력한 경우:

1. **모델 추론 (1차):** Claude 모델이 사용자 메시지를 분석하고, danny의 user_id를 모르므로 먼저 slack_search_users 도구를 호출하기로 결정합니다.
2. **도구 호출 (1차):** 앱이 MCP 서버로 slack_search_users({query: "danny"}) 요청을 전달하고, MCP 서버가 저장된 토큰으로 Slack API(users.list)를 호출하여 danny의 user_id를 반환합니다.
3. **모델 추론 (2차):** danny의 user_id를 확인한 모델이 slack_send_message 도구 호출을 결정합니다.
4. **도구 호출 (2차):** MCP 서버가 Slack API(chat.postMessage)를 호출합니다. 이때 토큰 소유자(Charles)의 신원으로 메시지가 전송되므로, Slack에서는 Charles가 보낸 DM으로 표시됩니다.
5. **응답 생성:** 모델이 도구 실행 결과를 바탕으로 자연어 응답("danny에게 메시지를 보냈습니다")을 생성하여 사용자에게 표시합니다.
...
Claude 데스크톱 앱에서 Slack으로 메시지를 전송할 때, 발신자가 기존 'Alex'에서 'Charles'로 변경된 것이 확인되었습니다.
...
Slack MCP 커넥터의 OAuth 재인증이 수행되었으며, 재인증 시점에 브라우저에 Alex가 아닌 Charles(s\*\*\*\*k) 계정으로 Slack에 로그인되어 있었던 것으로 추정됩니다. Phase 1의 3단계에서 설명한 바와 같이, 브라우저에 로그인된 계정이 곧 토큰 소유자가 되므로, 인증 계정이 변경된 것입니다.

### 5.4 복구 방법

1. Claude 데스크톱 앱에서 현재 Slack 커넥터를 해제(연결 끊기)합니다.
2. 웹 브라우저에서 Alex 계정으로 Slack에 로그인합니다.

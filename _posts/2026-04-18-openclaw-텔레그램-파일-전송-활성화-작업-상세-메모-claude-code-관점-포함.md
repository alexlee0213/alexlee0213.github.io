...
또 다른 심각한 문제는 assistant가 tool로 바로 조회 가능한 요청에도 실제 실행 없이 "아직 확인 안 했다"는 말만 반복하는 루프였다.

이를 막기 위해 다음 원칙을 추가했다.
- tool로 해결 가능한 요청이면 같은 턴에서 먼저 실행
- 실제 시도하지 않았으면 "아직 확인 안 했다"고 말하지 않음
- blocker가 있으면 구체적으로 설명
- 같은 non-result 답변 반복 금지

이건 product-level patch는 아니지만, visible failure mode를 강하게 줄이는 운영 규칙이었다.
